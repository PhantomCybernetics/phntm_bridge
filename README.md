...
Below is an example of config file generated in the previous step, e.g. `~/phntm_bridge.yaml`.
...
Add phntm_bridge service to your `~/compose.yaml` file with both `~/phntm_bridge.yaml` and `~/phntm_agent.yaml` mounted in the container as show below:
```yaml
services:
  phntm_bridge:
...
### Open the Web UI
Open `https://bridge.phntm.io/%ID_ROBOT%` in a web browser, this permanent link can be found at the top of the generated Bridge YAML config file.
(If you provided maintainer's e-mail in your config, the link will be sent to you for your reference after the first Bridge launch.) \
 \
